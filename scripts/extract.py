...
START_PATTERNS = [
    ...
    ("번호", re.compile(r"^\s*(\d{1,2})(?=\s+(?:[가-힣(]|[A-Za-z]{2}|[A-Z]\s|\d\s*,\s*\d|\d{1,3}\s*[이가]\s*적힌))"), "번호"),
    # '1 6명이 원탁에…' — 번호 뒤에 수량 숫자가 바로 오면 한글 요구 판이 못 잡는다
    # (확통 실측: 연습문제 번호 + '6명이'). 수량 단위가 붙을 때만 좁게 허용한다.
    ("번호", re.compile(r"^\s*(\d{1,2})"
                       r"(?=\s+\d{1,3}\s*(?:명|개|장|가지|마리|팀|자루|켤레|봉지|상자)[가-힣])"),
     "번호"),
]
    ...
    for i, ln in enumerate(live):
        ...
        if m[0] in WEAK_START:
            ...
            if NUMBER_FALSE_FOLLOW.match(txt(ln)):
                # '75 명이 가위바위보…' 는 번호 7 과 본문 '5명이' 가 붙은 오독이다
                # (확통 실측). 두 자리 + 수량 단위 꼴이면 살린다 — 번호는 틀리지만
                # 조판에서 순번을 다시 매기므로 문제 본문을 잃는 것보다 낫다.
                if not re.match(r"^\s*\d[1-9]\s*(?:명|개|장|가지|마리|팀|자루|켤레|봉지|상자)[가-힣]",
                                txt(ln)):
                    continue
